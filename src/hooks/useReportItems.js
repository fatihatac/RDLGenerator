...
//     // Eğer silinen öğe bir veri kaynağı ise, ona bağlı tabloyu da sil
//     if (itemToDelete && itemToDelete.type === 'data') {
//       setReportItems(prev => prev.filter(item =>
//         item.id !== id && // Veri kaynağını kaldır
//         !(item.type === 'table' && item.dataSourceId === id) // Bağlı tabloyu kaldır
//       ));
//     } else {
//       // Diğer tüm öğeler için standart silme işlemi
//       setReportItems(prev => prev.filter(item => item.id !== id));
//     }
//   };
...
//           type: "table",
//           columns: newColumns,
//           dataSourceId: updatedItem.id // <-- BAĞLANTIYI KUR
//         });
//       } else if (existingTable.columns.length === 0) {
//         itemsToUpdate[existingTable.id] = { 
//             columns: newColumns,
